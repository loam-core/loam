Loam Core Architecture Overview
A one‑page conceptual map of the identity‑native substrate.
Loam Core defines the physics of identity, continuity, policy, secrets, state, and execution. It does not define agent behavior — it defines the environment in which agents can exist.
Identity
The cryptographic root of self. An identity is:
    ...
    • a secrets store
    • a continuity chain
Identity is sovereign and immutable except through continuity. Identity is the actor, not the execution environment.
...
Loam Core provides three execution membranes:
